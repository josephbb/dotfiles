...
}:
{
  ...
  # Drop earlier nix→/usr/local/bin R shims; RStudio needs CRAN's R.framework (cask r-app).
  system.activationScripts.unlinkNixRForRStudio.text = ''
      ...
      if [ -L "$link" ]; then
        ...
        case "$target" in
          /etc/profiles/per-user/${username}/bin/*)
            echo "RStudio: removing nix shim $link"
            rm -f "$link"
            ;;
        esac
      fi
